...
static void RepRap12864_EnablePulse(void)
{
	...
	RepRap12864_TIM_MicorSecDelay(writeTimeConstant);
	HAL_GPIO_WritePin(PORT_RS_and_E, PIN_E, GPIO_PIN_RESET);
	RepRap12864_TIM_MicorSecDelay(60);
}
//2) RS control
...
{
	//Set RS to 0
	RepRap12864_RS(false);
	//Call low level write parallel function
	RepRap12864_write(command);
}
//6) Write 8 bits data
...
void RepRap12864_Begin8BIT(GPIO_TypeDef* PORT_RS_E, uint16_t RS, uint16_t E, GPIO_TypeDef* PORT_LSBs0to3, uint16_t D0, uint16_t D1, uint16_t D2, uint16_t D3, GPIO_TypeDef* PORT_MSBs4to7, uint16_t D4, uint16_t D5, uint16_t D6, uint16_t D7)
{
	...
	RepRap12864_writeCommand(LCD_FUNCTIONSET | LCD_FUNCTION_N | LCD_FUNCTION_DL);
	//4. Display control (Display ON, Cursor ON, blink cursor)
	RepRap12864_writeCommand(LCD_DISPLAYCONTROL | LCD_DISPLAY_B | LCD_DISPLAY_C | LCD_DISPLAY_D);
	//5. Clear LCD and return home
	RepRap12864_writeCommand(LCD_CLEARDISPLAY);
	HAL_Delay(2);
}
...
void RepRap12864_Begin4BIT(GPIO_TypeDef* PORT_RS_E, uint16_t RS, uint16_t E, GPIO_TypeDef* PORT_MSBs4to7, uint16_t D4, uint16_t D5, uint16_t D6, uint16_t D7)
{
	...
	HAL_Delay(20);
	//2. Attentions sequence
	RepRap12864_write4bitCommand(0x3);
	HAL_Delay(5);
	RepRap12864_write4bitCommand(0x3);
	HAL_Delay(1);
	RepRap12864_write4bitCommand(0x3);
	HAL_Delay(1);
	RepRap12864_write4bitCommand(0x2);  //4 bit mode
	HAL_Delay(1);
	...
	RepRap12864_writeCommand(LCD_FUNCTIONSET | LCD_FUNCTION_N);
	HAL_Delay(3);
}
//3) LCD print string
void RepRap12864_print(char string[])
{
	...
	{
		RepRap12864_writeData((uint8_t)string[i]);
	}
}
...
void RepRap12864_setCursor(uint8_t row, uint8_t col)
{
	...
	{
		maskData |= (0x80);
		RepRap12864_writeCommand(maskData);
	}
	else
	{
		maskData |= (0xc0);
...
{
	RepRap12864_setCursor(1,1);
}
void RepRap12864_2ndLine(void)
{
	RepRap12864_setCursor(2,1);
}
void RepRap12864_3rdLine(void)
{
	RepRap12864_setCursor(3,1);
}
void RepRap12864_4thLine(void)
{
	RepRap12864_setCursor(4,1);
}
//5) Enable two lines
void RepRap12864_TwoLines(void)
{
	FunctionSet |= (0x08);
	RepRap12864_writeCommand(FunctionSet);
}
...
{
	DisplayControl &= ~(0x02);
	RepRap12864_writeCommand(DisplayControl);
}
